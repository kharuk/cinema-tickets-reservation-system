...
class ReservationHelpers {
  countTotalPrice = (
    ...
    const priceOfTickets = chosenSeats.reduce(
      (sum, current) => sum + sessionSeatTypes[current.type],
      0
    );

    const priceOfExtraServices = Object.keys(chosenExtraServices).reduce(
      (sum, current) =>
        sum + chosenExtraServices[current] * extraServices[current],
      0
    );
    const totalPrice = priceOfTickets + priceOfExtraServices;
    return {
      priceOfTickets,
      priceOfExtraServices,
      totalPrice
    };
  };
